...
function parseCoordinators(raw?: string | null): string[] {
  ...
  const entries = raw
    ...
    .map((s) => s.trim())
    .filter(Boolean);
  return entries.length > 0 ? entries : [raw.trim()];
}
            ...
            >
              ...
              <div className="w-full lg:w-1/2 lg:h-full overflow-y-auto p-5 sm:p-7 lg:p-8 space-y-5 min-w-0">
                ...
                <div className="space-y-3">
                  {[
                    { label: "Entry Fee", value: `₹${activeEvent.fee.toLocaleString("en-IN")}`, isFee: true },
                    { label: "Date", value: activeEvent.date },
                    { label: "Day", value: activeEvent.day },
                    ...
                    { label: "Faculty Coordinator", value: activeEvent.facultyCoordinator, isCoordinator: true },
                    { label: "Student Coordinator", value: activeEvent.studentCoordinator, isCoordinator: true },
                  ].filter((item) => item.value && item.value !== "TBA" && item.value !== "NA").map((item) => (
                    <div
                      key={item.label}
                    ...
                    >
                      ...
                      {item.isCoordinator ? (
                        <div className="flex flex-col gap-1 min-w-0 flex-1">
                          {parseCoordinators(item.value).map((coordinator, idx) => (
                            <span
                              key={idx}
