...
import { PassPurchaseDetailModal } from "../components/PassPurchaseDetailModal";
...
export function AdminPassesTab({
  ...
  const handleSavePassPrice = async () => {
    ...
    try {
      ...
      if (onPassPriceUpdated) onPassPriceUpdated(res.data.pass);
      setEditingPass(null);
  ...
  return (
    <div className="space-y-4">
      ...
      {/* ── FESTIVAL PASS DYNAMIC PRICING SECTION ── */}
      {primaryPass && (
        <Card className="border border-white/[0.08] bg-[#140f26]/90 backdrop-blur-md shadow-xl overflow-hidden relative">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-5">
              <div className="space-y-1">
                ...
                <p className="text-xs text-white/50 max-w-xl">
                  Authoritative pass price stored in database and used by the public checkout and payment gateway.
                </p>
              </div>
              ...
              <div className="flex items-center gap-4 sm:border-l sm:border-white/[0.08] sm:pl-6">
                <div className="text-right sm:text-left">
                  ...
                  <span className="text-2xl sm:text-3xl font-black text-euphoria-gold tracking-tight block mt-0.5">
                ...
                <Button
                  ...
                  className="bg-euphoria-gold text-black hover:bg-euphoria-gold/90 font-bold text-xs gap-1.5 px-3.5 cursor-pointer shadow-lg shadow-euphoria-gold/20"
                >
      ...
      {/* ── EDIT FESTIVAL PASS PRICE DIALOG ── */}
      <Dialog open={!!editingPass} onOpenChange={(open) => !open && setEditingPass(null)}>
        <DialogContent className="bg-[#16112c] border-white/15 text-white max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-white text-base">
              <Ticket className="size-4 text-euphoria-gold" />
              Configure Festival Pass Price
            </DialogTitle>
            <DialogDescription className="text-white/60 text-xs">
              Update the authoritative price for {editingPass?.name}. This price takes effect immediately for new pass purchases.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-2">
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  className="pl-7 bg-black/40 border-white/15 text-white font-mono text-sm disabled:opacity-40"
                />
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 pt-1">
              <input
                ...
                checked={isPassTba}
                onChange={(e) => {
                  setIsPassTba(e.target.checked);
                  if (e.target.checked) setPassPriceInput("");
                }}
                className="rounded border-white/20 bg-black/40 text-euphoria-gold focus:ring-euphoria-gold size-4 cursor-pointer"
              />
              <label htmlFor="pass-tba" className="text-xs text-white/70 cursor-pointer select-none">
                Mark price as <span className="font-semibold text-white">TBA</span> (To Be Announced)
              </label>
            </div>
          </div>
